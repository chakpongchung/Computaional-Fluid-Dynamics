...
%
  ...
  fprintf ( 1, '  A1(X)  = 1.0\n' );
  fprintf ( 1, '  C1(X)  = 1.0\n' );
  fprintf ( 1, '  F1(X)  = -cos(pi*X)*cos(pi*X)-2*pi*pi*cos(2*pi*X)\n' );
...
%
  value = 2*x./(1+x);  % ii
  %value = 1.0 ./(1+exp(1)) .*exp(x)+ exp(1)/(1+exp(1)).* exp(-x)- cos(pi*x).*cos(pi*x);
...
%
  value = 2.0 ./( (1+x).*(1+x) );  %ii
  %value=1.0 ./(1+x) .*exp(x)- exp(1)/(1+exp(1)).* exp(-x)+ 2* pi*sin(pi*x).*cos(pi*x); % i
...
%
  value = 2./(1+x);  %ii
    %value =-cos(pi*x).*cos(pi*x)-2*pi*pi*cos(2*pi*x)
  return
end
